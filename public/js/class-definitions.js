...
function purchaseLaptop(laptopName) {
  if(!laptopCosts.hasOwnProperty(laptopName)) {
    return -1;
  }
  return laptopCosts[laptopName].toString();
}
...
Scientist.prototype.addDiscovery = function(discovery) {
  ...
  return "I discovered " + this.discoveries.filter(function(element, index, array) {
    return index < (array.length - 1);
  }).join(', ') + ', and ' + this.discoveries[this.discoveries.length - 1] + '.';
};
...
BankAccount.prototype.withdraw = function(person) {
  if(person === owner) {
    person.earnMoney(this.balance);
    this.balance = 0;
  }
};
BankAccount.prototype.deposit = function(person) {
  this.balance = person.money;
  person.money = 0;
};
...
// Create 2 boxes
var catBox = new Box(new Animal('Cat', gender.female), true);
var christmasPresent = new Box('gift card', false);
...
var notWarmOrColdAnimal = new Animal('Dinosaur', gender.male).isWarmBlooded();
...
var shortStory = singleStory.isATallStory(3);
